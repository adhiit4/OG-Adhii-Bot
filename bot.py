...
async def on_message(message):
    ...
    if xp_level(new_xp) > xp_level(old_xp):
        try:
            await message.channel.send(
                f"🎉 {message.author.mention} "
                f"reached **Level {xp_level(new_xp)}**!"
            )
        except Exception:
            pass
...
async def slowmode(
    ...
    try:
        ...
        await reply(
            interaction,
            f"❌ Slowmode failed: `{type(e).__name__}`",
            ephemeral=True
        )
...
async def createchannel(
    ...
    try:
        ...
        await reply(
            interaction,
            f"❌ Create failed: `{type(e).__name__}`",
            ephemeral=True
        )
...
async def renamechannel(
    ...
    try:
        ...
        await reply(
            interaction,
            f"❌ Rename failed: `{type(e).__name__}`",
            ephemeral=True
        )
...
async def autorole(
    ...
    if not me or role >= me.top_role:
        return await reply(
            interaction,
            "❌ The role must be below my bot role.",
            ephemeral=True
        )
...
async def server(interaction):
    ...
    embed.add_field(
        name="Channels",
        value=str(len(g.channels))
    )

    embed.add_field(
        name="Roles",
        value=str(len(g.roles))
    )
...
class Ticket(discord.ui.View):
    ...
    async def create_ticket(
        ...
        try:
            ...
            await reply(
                interaction,
                "❌ Discord denied the ticket creation. "
                "Give the bot **Manage Channels** permission "
                "and move its role high enough.",
                ephemeral=True
            )
...
@tree.command(
    name="setupsupport",
    description="Create or repair the complete ticket support system"
)
@app_commands.checks.has_permissions(
    manage_channels=True
)
async def setupsupport(
    interaction: discord.Interaction
):
    guild = interaction.guild
...
async def help_command(interaction):
    embed = discord.Embed(
        ...
        description=(
            ...
            "Mute • Clear • Lock • Unlock • Slowmode\n\n"
