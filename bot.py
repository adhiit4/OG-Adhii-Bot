...
async def setvc(interaction: discord.Interaction):
    ...
    if not interaction.user.voice or not interaction.user.voice.channel:
        return await reply(
            interaction,
            "❌ Join the voice channel first, then use `/setvc`.",
            ephemeral=True,
        )

    channel = interaction.user.voice.channel
    if not isinstance(channel, discord.VoiceChannel):
        return await reply(interaction, "❌ Please use a normal voice channel.", ephemeral=True)
    ...
    if not permissions.connect: 
        return await reply(interaction, "❌ I need **Connect** permission in that VC.", ephemeral=True)

    if not permissions.speak:
        # The bot does not play audio, so Speak is not required.
        pass
    ...
    try:
        ...
        await reply(
            interaction,
            f"🔊 **24/7 VC set!**\nBot will stay in {channel.mention} and reconnect after a restart.",
            ephemeral=True,
        )
    except discord.Forbidden:
        await reply(
            interaction,
            "❌ Discord denied the VC connection. Check **Connect** permission.",
            ephemeral=True,
        )
    except Exception as e:
        print("SETVC ERROR:", repr(e))
        await reply(
            interaction,
            f"❌ VC connection failed: `{type(e).__name__}`",
            ephemeral=True,
        )
...
async def leavevc(interaction: discord.Interaction):
    ...
    await reply(interaction, "🔇 24/7 VC disabled for this server.", ephemeral=True)
